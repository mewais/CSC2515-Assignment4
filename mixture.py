...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        R_sum_1d = R.sum(axis=0)            # Sum R over i
        R_sum_2d = R.sum()                  # Sum R over i and k
        A = self.prior.a_mix                # the A matrix
        K = R.shape[1]                      # Second dimension of R (Needed because we sum a constant over that dim)
        Numerator = R_sum_1d + A - 1;       # Our Numerator
        Denominator = R_sum_2d + K*A - K;   # Our Denominator (uses the K constant generated above)
        PI = Numerator / Denominator        # The PI value
        return PI
        #################################################################
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        a = self.prior.a_pixels             # The a constant
        b = self.prior.b_pixels             # The b constant
        R_tran = R.transpose()              # A transpose of our R matrix, needed to multiply by X
    ...
    def compute_posterior(self, X, M=None):
        ...
        ######################## Your code here #########################
        PI = self.params.pi                 # PI
        Theta = self.params.theta           # Theta
        RemTheta = 1 - Theta                # Remainder Theta
        ObsX = X * M                        # Observed Xs
        RemObsX = 1 - ObsX                  # Remainded Observed Xs
        Numerator = np.log(PI) + np.dot(ObsX, np.log(Theta).transpose()) + np.dot(RemObsX, np.log(RemTheta).transpose())
        # We don't really need to caculate the denominator
        # Denominator = np.logaddexp.reduce(Numerator, axis=1)
        log_p_z_x = Numerator # / Denominator
        #################################################################
    ...
    def posterior_predictive_means(self, X, M):
        ...
        PredMeans = np.matmul(Posterior, Theta)
        return PredMeans
        #################################################################
